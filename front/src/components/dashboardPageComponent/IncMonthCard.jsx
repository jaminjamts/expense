...
import { DotIcon, IncreaseIcon } from "@/icons";
import { useEffect, useState } from "react";

export const IncMonthCard = ({ userID }) => {
  ...
  const monthDatas = async () => {
    ...
    try {
      const response = await fetch(
        `${BACKEND_ENDPOINT}/monthTransactions?userID=${userID}&types=INC&start=${start.toISOString()}`
      );
